...
data_wide[8:33] <- lapply(data_wide[8:33], as.numeric)
dim(data_wide)
...
```{r}
data[8:37] <- apply(data[8:37], 2, function(x) {ifelse(x < 0, NA, x)}) 
data[8:37] <- lapply(data[8:37], as.numeric)

newdata <- newdata[!is.na(newdata$CHILDID),]
summary(newdata$EXPECT)

if(data$round==2 | data$round==5){
data <- data %>%
  mutate(trip = CONCRT+MUSEUM+ZOO+SPORT)
}
